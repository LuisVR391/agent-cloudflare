...
/**
 * Por qué una salida quedó inválida, cuando el motivo cambia el desenlace de
 * quien llama. Hoy hay uno solo: el modelo pidió una herramienta en una llamada
 * que no anunció ninguna. El proveedor sabe que esa llamada fue sin
 * herramientas, pero no por qué —la corrida no tiene ninguna autorizada, o
 * gastó su presupuesto de rondas—, y son dos causas distintas que merecen dos
 * códigos distintos. El código del proveedor no cambia; con qué cierra la
 * corrida lo decide quien sabe por qué la llamada fue desnuda.
 */
export type ModelInvalidOutputReason = "TOOL_CALL_NOT_OFFERED";

export class ModelProviderError extends Error {
  readonly code: ModelFailureCode;
  /** Nulo salvo cuando el motivo distingue dos desenlaces para quien llama. */
  readonly reason: ModelInvalidOutputReason | null;

  constructor(
    code: ModelFailureCode,
    options?: { cause?: unknown; reason?: ModelInvalidOutputReason },
  ) {
 ...
 *
 * `parameters` es el segundo rótulo del mismo dato: es el que el modelo escribe
 * cuando redacta la petición él mismo, en vez de emitirla por el canal que el
 * proveedor reconoce. Aceptarlo no relaja nada, porque lo que decide si esos
 * argumentos valen sigue siendo el schema del catálogo.
 */
...
/**
 * El sobre del proveedor. `response` no se declara como texto porque no siempre
 * lo es: cuando el modelo pide una herramienta, Workers AI deja en ese campo el
 * objeto de la función que el modelo escribió y entrega `tool_calls` vacío.
 * Declararlo como cadena convertía esa salida —legítima, y la más frecuente del
 * modelo con function calling— en `MODEL_OUTPUT_INVALID`, perdiendo la
 * respuesta entera. Qué es cada cosa lo decide `parseModelReply`, que es quien
 * puede distinguir un texto de una petición.
 */
...
/**
 * El texto de una respuesta, con el límite que el canal impone. Se valida
 * aparte para que la garantía siga siendo la de siempre: una respuesta vacía o
 * más larga de lo que WhatsApp acepta no se recorta ni se rellena, falla
 * cerrado.
 */
...
/**
 * La petición de herramienta que el modelo redactó como contenido, una o
 * varias. Es la misma forma de siempre: lo único distinto es por dónde llegó.
 */
const writtenToolCallsSchema = z.union([
  toolCallSchema.transform((call) => [call]),
  z.array(toolCallSchema).min(1),
]);

/**
 * La petición redactada, venga ya deserializada o dentro de la cadena que el
 * proveedor no abrió. El sobre medido trae ese mismo contenido dos veces —como
 * objeto en `response` y como texto en `choices[0].message.content`—, así que
 * un proveedor que entregue la cadena está describiendo exactamente lo mismo:
 * leerla como respuesta enviaría `{"type":"function",…}` a la clienta.
 *
 * La regla es estrecha a propósito. No basta con que el texto parezca JSON:
 * tiene que parsear **y** casar con la forma de una llamada, cuyo nombre es un
 * identificador acotado. Una respuesta legítima que empiece por `{` no casa, y
 * se sigue enviando tal cual. Devuelve `null` cuando el candidato no describe
 * ninguna petición.
 */
function readWrittenToolCalls(candidate: unknown): ModelToolCall[] | null {
  let source = candidate;
  if (typeof candidate === "string") {
    try {
      source = JSON.parse(candidate);
    } catch {
      return null;
    }
  }
  const written = writtenToolCallsSchema.safeParse(source);
  return written.success ? written.data.map(toToolCall) : null;
}
  ...
  if ("function" in candidate) {
    return {
      ...
      arguments: parseToolArguments(
        candidate.function.arguments ?? candidate.function.parameters,
      ),
    };
  }
  return {
    id: null,
    name: candidate.name,
    arguments: parseToolArguments(candidate.arguments ?? candidate.parameters),
  };
}
 ...
 *
 * Una salida sin texto no es lo mismo que una respuesta vacía. El modelo que
 * está pidiendo herramientas no responde texto porque todavía no tiene qué
 * responder, y su petición puede llegar por dos caminos: el campo que el
 * proveedor reconoce, o el contenido que el modelo escribió cuando el proveedor
 * no la reconoció. Ambos describen lo mismo, y perder el segundo es perder la
 * respuesta entera. Lo que sigue siendo inválido es no traer ninguna de las
 * dos, y una respuesta de texto vacía cuando el modelo debía responder.
 */
export function parseModelReply(
  ...
  // Lo que no llegó por `tool_calls` todavía puede ser la petición que el
  // modelo redactó, y el proveedor la deja unas veces como objeto y otras como
  // la cadena JSON sin abrir. Se lee igual en los dos casos: la petición sigue
  // el camino de la petición, con los límites de ronda y de llamadas de la
  // corrida, en vez de viajar a la clienta como si fuera una respuesta.
  if (calls.length === 0 && response !== null && response !== undefined) {
    const written = readWrittenToolCalls(response);
    if (written !== null) {
      calls.push(...written);
    } else if (typeof response !== "string") {
      // Lo que no es texto y tampoco describe una petición no es nada
      // ejecutable, y no se interpreta: falla cerrado en vez de acabar como
      // mensaje hacia la clienta. Un texto que no casa sí es una respuesta
      // legítima, y lo decide el schema del canal unas líneas más abajo.
      throw new ModelProviderError("MODEL_OUTPUT_INVALID");
    }
  }
  ...
  if (calls.length > 0) {
    if (options.toolsOffered !== true) {
      // El motivo viaja con el error porque el desenlace depende de él: esta
      // llamada no anunció herramientas, y solo quien la hizo sabe si fue
      // porque la corrida no tiene ninguna o porque se le acabaron las rondas.
      throw new ModelProviderError("MODEL_OUTPUT_INVALID", {
  ...
  const text = replyTextSchema.safeParse(response);
  if (!text.success) {
    throw new ModelProviderError("MODEL_OUTPUT_INVALID");
  }
  return { kind: "text", text: text.data };
}
